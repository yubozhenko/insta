require_relative '../app/const/devices'
...
class DeviceGenerator
  ...
  def compute_hash(data)
    OpenSSL::HMAC.hexdigest OpenSSL::Digest.new('sha256'), Defaults::PRIVATE_KEY[:SIG_KEY], data
  end
  ...
  def info
    line = Devices.devices[md5int % Devices.devices.count]
    {
        manufacturer: line[0],
        device: line[1],
        model: line[2]
  ...
  def useragent_hash
    ...
    {
        agent: agent.join('; '),
        version: Defaults::PRIVATE_KEY[:APP_VERSION]
    }
  end
